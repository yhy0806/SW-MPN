...
from .utils1 import do_mixup, interpolate


def window_partition(x, window_size):
    B, H, W, C = x.shape
    x = x.view(B, H // window_size, window_size, W // window_size, window_size, C)
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size, C)# windows: (num_windows*B, window_size, window_size, C)
    return windows
...
def window_reverse(windows, window_size, H, W):
    ...
    x = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(B, H, W, -1) # x: (B, H, W, C)
    return x
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def plot_spectrogram(spectrogram, title="Spectrogram", ylabel="Frequency", xlabel="Time"):
        # Remove batch and channel dimensions, assuming spectrogram shape is (batch_size, 1, time_steps, freq_bins)
        spectrogram = spectrogram.squeeze()  # Reduce dimensions if necessary

        plt.figure(figsize=(10, 6))
        plt.title(title)
        plt.ylabel(ylabel)
        plt.imshow(spectrogram.T, aspect='auto', origin='lower')  # Transpos
        plt.xlabel(xlabel)
        plt.colorbar(format="%+2.0f dB")
        plt.show()

    def forward(self, x: torch.Tensor, mixup_lambda=None, infer_mode=False, return_feature=False):  # out_feat_keys: List[str] = None):
        x = self.spectrogram_extractor(x)  # (batch_size, 1, time_steps, freq_bins)
        #spectrogram_data = x[0]  # Select the first sample in the batch for visualization
        #self.plot_spectrogram(spectrogram_data, title="Spectrogram (Frequency Bins)")
        ...
        feature = output_dict['latent_output']
